...
#ifndef _RENDERER_DISTANCE_ESTIMATION_H
#define _RENDERER_DISTANCE_ESTIMATION_H
...
// These functions return the min or max of two numbers, but with an adjustable
// amount of polynomial smoothing to make the transition smooth.
double smooth_min(double a, double b, double k);
double smooth_max(double a, double b, double k);

// Represents a 3D scene as a distance function - a function that, given a
// vertex, returns the distance from the vertex to the closest point in the
// scene.
...
/**
 *  Estimates the normal of a distance field at a given point using a numerical
 * gradient.
 *
...
/**
 *  Intersects a ray with a 3D scene given as a distance function using ray
 * marching.
 *
 *  @param r             A ray to intersect with the scene
 *  @param obj           The 3D scene to render, as a distance function
 *  @param t             If the ray intersects, this stored the distance of the
 * intersection along the ray from the ray's origin.
 *  @param steps         Stores number of steps ray was marched before
 * intersecting.
 *  @param max_ray_steps Maximum number of steps to march the ray before
 * assuming it does not intersect.
 *  @param .001          Tolerance for a ray to intersect with the scene.
 *
 *  @return Whether or not the ray intersects with the scene.
 */
bool ray_march(const ray &r, const distance_estimator &obj, double *t,
               int *steps, int max_ray_steps, double epsilon = 0.0001);

#endif /* _RENDERER_DISTANCE_ESTIMATION_H */
