...
RenderArea::RenderArea(QWidget *parent): QWidget(parent){
    ...
    sc = new scene(new camera(), std::vector<lamp*>(), new world(), new mesh(infile, "Object"));
    clock_t begin = clock();
...
void RenderArea::updateText(){
    switch(rendermode){
        case 0:
            ...
            break;
        case 1:
            statuslbl->setText(QString("2. ZBuffer draw, SSE") + QString(sc->obj->smooth ? ", smooth" : ""));
            break;
        case 2:
            statuslbl->setText(QString("3. SSAO") + QString(sc->obj->smooth ? ", smooth" : ""));
            break;
        case 3:
            statuslbl->setText(QString("4. Normal map") + QString(sc->obj->smooth ? ", smooth" : ""));
            break;
        case 4:
...
void RenderArea::keyPressEvent(QKeyEvent *event){
    switch(event->key()){
        ...
        case Qt::Key_O:
            ...
            break;
//        case Qt::Key_R:
//            //add threading for render
//            break;
        case Qt::Key_S:
            sc->obj->smooth = !sc->obj->smooth;
            updateText();
            updateImage();
            break;
        case Qt::Key_Z:
...
void RenderArea::zBufferDraw(){
    ...
    for(face *f : sc->obj->faces){
        ...
        fcenter = f->center();
        col = calcLighting(fcenter, f->normal, *sc->obj->mat, sc);
...
void RenderArea::generate_maps_vector(int mapflags){
    ...
    for(face *f : sc->obj->faces){
        ...
        if((mapflags & (4+2)) && sc->obj->smooth){
            norm = f->vertices[0]->vertexNormal();
            norm2 = f->vertices[1]->vertexNormal();
            norm3 = f->vertices[2]->vertexNormal();
            col = calcLighting(*f->vertices[0], norm, *sc->obj->mat, sc);
            col2 = calcLighting(*f->vertices[1], norm2, *sc->obj->mat, sc);
            col3 = calcLighting(*f->vertices[2], norm3, *sc->obj->mat, sc);
        }
            ...
            for(pint.x=minx; pint.x<=maxx; pint.x+=EdgeVect::stepX){
                ...
                for(i=0; i<4; i++){
                    if(pint.x+i<w && pxmask[i]!=0 && z[i] < zbuffer[w*pint.y+pint.x+i]){
                        if(mapflags & 2){
                            if(sc->obj->smooth) normtmp = lerp(norm, norm2, norm3, real(w1[i])/w0[i], real(w2[i])/w0[i], real(w3[i])/w0[i]);
                            else normtmp = f->normal;
                            normalmap[pint.y*w + pint.x+i] = normalToRGB(normtmp);
                        }
                        if(mapflags & 4){
                            if(sc->obj->smooth) colrgb = colorToRGB(lerp(col, col2, col3, real(w1[i])/w0[i], real(w2[i])/w0[i], real(w3[i])/w0[i]));
                            else if(colrgb>>24) colrgb = colorToRGB(calcLighting(fcenter, f->normal, *sc->obj->mat, sc));
